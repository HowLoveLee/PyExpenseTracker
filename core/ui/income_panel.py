...
class IncomePanel(QWidget):
    ...
    def setupIncomeTable(self, table):
        ...
        table.setColumnWidth(DATE_RECEIVED_COLUMN, 120)
        table.setColumnWidth(AMOUNT_COLUMN, 100)
        table.setColumnWidth(COMMIT_COLUMN, 100)
        ...
        for i in range(3):
            for j in range(5):
                ...
                elif j == AMOUNT_COLUMN:
                    item = QTableWidgetItem()
                    item.setText("0.00")
                    item.setTextAlignment(Qt.AlignmentFlag.AlignRight)
                    table.setItem(i, j, item)
                elif j == COMMIT_COLUMN:
        ...
        table.setItemDelegateForColumn(DATE_RECEIVED_COLUMN, date_delegate)
    ...
    def commitAllButtonClicked(self):
        for row in range(self.incomeTable.rowCount()):
            commitButton = self.incomeTable.cellWidget(row, 4)

            if not commitButton.isEnabled():
                continue

            name_item = self.incomeTable.item(row, 1)
    ...
    def receiptButtonClicked(self, button):
        ...
        if filePath:  # If a file was selected
            destinationFolder = "C:\Dev\PythonProjects\TheMonkeyTracker\Receipts" #Hardcoding cringe
